...
`pip install ostia` installs the full open stack. Python packages share one import namespace (PEP 420): each component installs into `ostia/<component>/`, and no distribution ships `ostia/__init__.py`, so any combination of components can be installed side by side. Shared libraries follow the component names, e.g. `libostia-fabric.so`.
...
| D11 | Product name | Ostia, after ancient Rome's harbour (Portus and Cursus rejected because of existing projects in the same space). Status on 2026-09-25: GitHub org `OstiaHQ` created. PyPI: `ostia`, `ostia-fabric`, `ostia-exchange` and `ostia-query` claimed with 0.0.1 placeholder releases; `ostia-runtime`, `ostia-telemetry` and `ostiadb` follow once PyPI's new-project rate limit resets. conda-forge names can't be reserved; the `ostia` recipe is submitted with the first real release. Trademark: preliminary screening done in the US, EU and WIPO registers; the filing strategy is decided with counsel before filing. Domains: none registered yet; `ostiahq.com` and `ostiadb.dev` were available | Decided |
